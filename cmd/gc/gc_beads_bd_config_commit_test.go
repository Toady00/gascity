...
func configCommitHarness(t *testing.T) string {
	...
	// tests fail on the behaviour they assert (no commit was issued) rather
...
// containing DOLT_COMMIT fails with that text, modelling Dolt's refusal to
...
func runEnsureConfigValue(t *testing.T, db, key, value string, env ...string) (string, []string, error) {
	t.Helper()
	dir := t.TempDir()
	...
	cmd := exec.Command("bash", harnessPath, db, key, value)
...
func TestEnsureBdRuntimeConfigValueCommitsTheWrite(t *testing.T) {
	out, queries, err := runEnsureConfigValue(t, "rigdb", "issue_prefix", "sctforg")
	if err != nil {
...
func TestEnsureBdRuntimeConfigValueCommitScopedToConfigTable(t *testing.T) {
	_, queries, err := runEnsureConfigValue(t, "rigdb", "issue_prefix", "sctforg")
	if err != nil {
...
func TestEnsureBdRuntimeConfigValueCommitsCustomTypes(t *testing.T) {
	_, queries, err := runEnsureConfigValue(t, "rigdb", "types.custom", "molecule,convoy,session")
	if err != nil {
		t.Fatalf("ensure_bd_runtime_config_value failed: %v", err)
	}
	joined := strings.Join(queries, "\n")
...
func TestEnsureBdRuntimeConfigValueReportsCommitFailure(t *testing.T) {
	out, _, err := runEnsureConfigValue(t, "rigdb", "issue_prefix", "sctforg",
