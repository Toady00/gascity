...
func TestEffectiveWorkQueryNamedSingletonGraphAnchor(t *testing.T) {
	for _, tc := range []struct {
		...
		{"singleton-ready", 1, true, "ready-step"},
		{"singleton-fallback", 1, false, "workflow-root"},
		{"pool-named-fallback", 2, true, "workflow-root"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			a := Agent{Name: "worker", Dir: "hello-world", MaxActiveSessions: &tc.max}
			out := runEffectiveWorkQuery(t, a, map[string]string{
				"GC_SESSION_NAME":   "worker-session",
				"GC_SESSION_ORIGIN": "named",
			}, graphWorkflowAnchorFakeBD(tc.ready))
			ids := workQueryOutputIDOrder(t, out)
			if len(ids) != 1 || ids[0] != tc.want {
				t.Fatalf("EffectiveWorkQuery() IDs = %v, want [%s]", ids, tc.want)
			}
		})
	}
}
