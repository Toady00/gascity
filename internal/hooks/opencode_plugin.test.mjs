...
//
// The OpenCode plugin registers hooks only when gc's session identity is in
// the environment (managedSessionIdentityPresent), so the behavioral suites
// run with GC_SESSION_ID set; the first test below pins the unmanaged no-op.
...
// A managed session identity, as the session lifecycle seeds it; without one
// the OpenCode factory returns no hooks at all.
process.env.GC_SESSION_ID = "gc-plugin-test-session";
...
test("OpenCode factory is silent without identity and registers managed hooks", async () => {
  const source = await readFile(plugins[0].source, "utf8");
