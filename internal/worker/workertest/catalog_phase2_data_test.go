...
func TestPhase2CatalogDataIsAuthoritative(t *testing.T) {
	expected := []Requirement{
		...
		{Code: RequirementInputHookPrimedResumeRoleOmitted, Group: "input_delivery", Description: "A resumed session whose installed provider hook supplies the rendered role prompt to every generation delivers the beacon plus the configured nudge (the beacon alone when none is configured) as its restart turn, never the rendered prompt; every other profile keeps the rendered prompt in the restart turn."},
